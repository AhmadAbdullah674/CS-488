...
void A0::appLogic()
{
	...
	m_shape_size = glm::clamp(m_shape_size, 1.0f, 10.0f);
...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		createRotationSliderModifyRotationVariable();
...
bool A0::windowResizeEvent(int width, int height) {
	...
	// Fill in with event handling code...
	m_shape_size = (float)m_windowWidth / (float)m_windowHeight;
	return eventHandled;
}
...
void A0::createRotationSliderModifyRotationVariable()
{
	ImGui::SliderFloat("Rotation Control", &m_shape_rotation, 0.0f, 360.0f);
}

void A0::resetButtonCreation()
{
	if (ImGui::Button("Reset")) {
		m_shape_rotation = 0.0f;
		m_shape_translation = { 0.0f,0.0f };
		m_shape_size = 1.0f;
		m_shape_color = { 1.0f,1.0f,1.0f };
	}
}
